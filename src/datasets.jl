...
Iris flower data set introduced by Ronald Fisher [1].
The matrix `X` contains the numeric measurements,
in which each individual column denotes an observation.
The vector `y` contains the class labels as strings.
The vector `vars` contains the names of the features (i.e. rows of `X`)
...
The vector `vars` contains descriptive names for `x` and `y`
"""
function load_sin()
